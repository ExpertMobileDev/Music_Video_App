...
#import "FullScreenIOS7ControlsView.h"
...
@interface FullScreenIOS7ControlsView() {
}
...
@implementation FullScreenIOS7ControlsView
...
- (void)initButtons {
     ...
     */
    NSString* doneButtonTitle = [[PlayerViewController currentLanguageSettings] objectForKey:@"Done"];
    ...
    _fixedSpace.width = [UIUtils isIpad] ? 20 : 10;
    ...
    _playButton = [[OOPlayPauseButton alloc] initWithScale:2.0];
    ...
    UIImage *image = [UIImage imageWithCGImage:[[ImagesIOS7 routeImage] CGImage]
                                         scale:_routeScale orientation:UIImageOrientationUp];
    UIImage *enabledImage = [UIImage imageWithCGImage:[[ImagesIOS7 routeOnImage] CGImage]
                                                scale:_routeScale orientation:UIImageOrientationUp];
    ...
    for (id current in _volumeButton.subviews) {
        if ([current isKindOfClass:[UISlider class]]) {
            ...
            // scrubber thumb
            UIImage *thumb = [ImagesIOS7 thumbImage];
    ...
    _volumeButton.tintColor = [UIColor blueColor];
...
- (void) updateToolbar {
    ...
    if (![UIUtils isIpad] && ([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationPortrait))  {
...
- (void)changeDoneButtonLanguage:(NSString*)language {
    _doneButton.title = [[PlayerViewController currentLanguageSettings] objectForKey:@"Done"];
}
