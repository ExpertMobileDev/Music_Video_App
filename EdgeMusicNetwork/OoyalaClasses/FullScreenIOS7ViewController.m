...
@interface FullScreenIOS7ViewController ()
...
@property (nonatomic) FullScreenIOS7ControlsView *controls;
@end

@implementation FullScreenIOS7ViewController
...
- (void)viewDidLoad {
  ...
  self.controls = [[FullScreenIOS7ControlsView alloc] initWithFrame:self.view.bounds];
  ...
  self.controls.doneButton.target = self;
...
- (void)viewDidAppear:(BOOL)animated {
  [self updateClosedCaptionsPosition];
  [[NSNotificationCenter defaultCenter] postNotificationName:PlayerViewControllerFullscreenViewVisible object:self];
}
...
-(void)onFullscreenDoneButtonClick {
    ...
    [self.delegate onFullscreenDoneButtonClick];
}
